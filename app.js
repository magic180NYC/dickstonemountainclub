...
function setupArchive(){
  ...
  document.addEventListener('click',e=>{
    ...
    if(!btn)return;
    clearSearchInput();
    if(btn.dataset.theme||btn.dataset.feature){btn.classList.toggle('active');applyFilters();return}
...
function courseDetailText(c,m){
  ...
  return '<div class="course-detail-meta"><span>거리 <b>'+escapeHtml(c.distance||m.distance||'-')+'</b></span><span>시간 <b>'+escapeHtml(c.time||m.time||'-')+'</b></span><span>난이도 <b>'+escapeHtml(c.level||m.level||'-')+'</b></span><span>상승 <b>'+escapeHtml(c.elevation||m.elevation||'-')+'</b></span></div><p>'+escapeHtml(c.summary||'코스 설명을 준비중입니다.')+'</p><div class="course-detail-section"><h4>이동 흐름</h4><div class="course-flow detail">'+(c.route||[c.start,c.mid,c.summit||m.name,c.end]).filter(Boolean).map((step,idx)=>'<span class="course-step"><i>'+(idx+1)+'</i>'+escapeHtml(step)+'</span>').join('<em>→</em>')+'</div></div><div class="course-detail-section"><h4>판단 포인트</h4><ul><li>추천: '+escapeHtml(targets)+'</li><li>노면: '+escapeHtml(terrain)+'</li><li>교통: '+escapeHtml(c.transport||'확인 필요')+'</li></ul></div><div class="course-mini-facilities"><span>화장실 <b>'+escapeHtml(c.toilet||m.facilities?.toilet||'확인 필요')+'</b></span><span>주차 <b>'+escapeHtml(c.parking||m.facilities?.parking||'확인 필요')+'</b></span><span>대중교통 <b>'+escapeHtml(c.transport||'확인 필요')+'</b></span></div><div class="course-detail-actions"><button class="btn primary" type="button" data-course-save>코스 저장</button><a class="btn kakao" href="'+mapUrl(m,'kakao')+'" target="_blank" rel="noopener">카카오맵</a><a class="btn naver" href="'+mapUrl(m,'naver')+'" target="_blank" rel="noopener">네이버지도</a></div>';
}
