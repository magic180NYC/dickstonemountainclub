...
document.addEventListener('DOMContentLoaded',async()=>{await loadMountains();const params=new URLSearchParams(location.search);const q=params.get('q');if(q&&$('[data-search-input]'))$('[data-search-input]').value=q;setupHomeSearch();setupClubBanner();setupArchive();setupLoginForm();if(q)applyFilters();try{await initAuth()}catch(e){console.warn('로그인 기능을 초기화하지 못했습니다. 검색/필터 등 나머지 기능은 정상 동작합니다.',e)}try{await renderDynamic()}catch(e){console.warn('저장/후기 데이터를 불러오지 못했습니다.',e)}});
